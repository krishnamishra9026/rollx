                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        ...
                        @if(session()->has('route'))
                          ...
                          @else
                              <a href="{{ route('admin.orders.create') }}" class="btn btn-sm btn-primary me-1"><i
                                class="mdi mdi-chevron-double-left me-1"></i>Back</a>
                        @endif
                        ...
                        <div class="row">
                            <div class="col-12">
                                <ul class="nav nav-tabs nav-bordered mb-3">
                                    <li class="nav-item">
                                        <a href="{{ route('admin.orders.edit', $order->id) }}" class="nav-link">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane show active" id="order-info-tab">
                                        <form id="orderForm" method="POST"
                                            action="{{ route('admin.orders.save-equipment-info', $order->id) }}"
                                            enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            @if($order->equipment_assemble_type == 'inventory')
                                            <input type="hidden" name="type_of_equipment" value="existing">
                                            @else
                                            <input type="hidden" name="type_of_equipment" value="new">
                                            @endif
                                            @if($order->equipment_assemble_type == 'inventory')
                                            <div class="row mb-2">
                                                <label for="existing_model"
                                                    class="col-md-3 col-form-label text-md-start">{{ __('Equipment Model') }}</label>

                                                <div class="col-md-9">
                                                    <select id="existing_equipment" class="form-control @error('existing_equipment') is-invalid @enderror" name="existing_equipment" placeholder="Enter Choose Equipment" onchange="getSerialNumbers()">
                                                        <option value="">Choose Equipment Model</option>
                                                        @foreach ($equipments as $equipment)
                                                            <option value="{{ $equipment->id }}" {{ old('existing_equipment', $order->existing_equipment) == $equipment->id ? "selected" : "" }}>{{ $equipment->equipment_name }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="col-md-9">
                                                    <select id="serial_number"
                                                        class="form-select @error('serial_number') is-invalid @enderror"
                                                        name="serial_number">
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="col-md-9">
                                                    <input id="installation_date" type="text"
                                                        ...
                                                        value="{{ old('installation_date', \Carbon\Carbon::parse($order->installation_date)->format("d-m-Y")) }}">
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="col-md-3" id="warranty_date" style="display:none;">
                                                    <input  type="text"
                                                        ...
                                                        value="{{ old('warranty_date', \Carbon\Carbon::parse($order->warranty_date)->format("d-m-Y")) }}">
                                                </div>
                                            </div>
                                            ...
                                            <div class="row mb-2" id="service_contract_row">
                                                ...
                                                <div class="col-md-4">
                                                    <input id="service_start_date" type="text"
                                                        ...
                                                        value="{{ old('service_start_date', \Carbon\Carbon::parse($order->service_start_date)->format("d-m-Y")) }}">
...
@push('scripts')
    ...
    {{-- <script>
        $(document).ready(function () {
            chooseEquipmentModel();
        });
    </script>
    <script>
        function chooseEquipmentModel(){
            let type = $("#type_of_equipment").val();
            switch (type) {
                case 'existing':
                    $("#existing-equipment-div").show();
                    $("#new-equipment-div").hide();
                    break;

                default:
                $("#existing-equipment-div").hide();
                $("#new-equipment-div").show();
                    break;
            }
        }
    </script> --}}
    ...
    <script>
        function getSerialNumbers() {
            let equipment_id = $("#existing_equipment").val();
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                }
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.inventory-equipment.get-serials') }}",
                data: formData,
                dataType: 'json',
                success: function (data) {
                    $('#serial_number').html(data.html);
