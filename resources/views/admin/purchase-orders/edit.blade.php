...
@section('content')
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-9">
                                    <input id="order_date" type="text"
                                        ...
                                        value="{{ old('order_date', \Carbon\Carbon::parse($purchase_order->order_date)->format("d-m-Y")) }}">
                                    ...
                                    <label class="border mb-2 me-1" for="images">
                                        ...
                                        <span class="uploaded-gallery">
                                            ...
                                            <span class="image-container">
                                                <img class="image" src="{{ asset('storage/uploads/purchase-orders/'.$purchase_order->id.'/images'.'/'.$image->name) }}" width="150px" height="100px" class="me-2 mt-2 mb-2 border">
                                                <div class="image-delete-button">
                                                    <a href="{{ asset('storage/uploads/purchase-orders/'.$purchase_order->id.'/images'.'/'.$image->name) }}" download="{{ $image->name }}" class="btn btn-sm btn-dark"><i class="uil-image-download"></i> </a>
                                            ...
                                            @foreach($purchase_order->documents as  $document)
                                                <span class="document-container me-2 mt-2 mb-2 border text-center py-4" style="width: 150px; height: 100px;">
                                                    ...
                                                    <div class="document-delete-button">
                                                        <a href="{{ asset('storage/uploads/purchase-orders/'.$purchase_order->id.'/documents'.'/'.$document->name) }}" download="{{ $document->name }}" class="btn btn-sm btn-dark"><i class="uil-file-download-alt"></i> </a>
    ...
    <form id='delete-document-form{{ $document->id }}'
        action='{{ route('admin.purchase-orders.delete-document', $document->id) }}'
        method='POST'>
    ...
    <form id='delete-image-form{{ $image->id }}'
        action='{{ route('admin.purchase-orders.delete-image', $image->id) }}'
        method='POST'>
        <input type='hidden' name='_token'
            value='{{ csrf_token() }}'>
        <input type='hidden' name='_method' value='DELETE'>
