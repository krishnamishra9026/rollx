...
@section('content')
    <div class="container-fluid Purchase_Orders">
        ...
        @include('admin.products.franchise.reports.filter')
                                        ...
                                        <table id="basic-datatable" class="table table-striped border dt-responsive nowrap w-100" style="font-size: 13px;">
                                            ...
                                            <tbody>
                                                @foreach ($sales as $report)
                                                    @php
                                                        $franchiseCount = $report->franchises->count();
                                                    @endphp
                                                    @if ($franchiseCount > 0)
                                                        @foreach ($report->franchises as $index => $franchise)
                                                            <tr>
                                                                @if ($index === 0)
                                                                    <td rowspan="{{ $franchiseCount }}">
                                                                        <a href="{{ route('admin.products.show', $report->id) }}">{{ $report->name }}</a>
                                                                    </td>
                                                                @endif
                                                                <td>
                                                                ...
                                                                <td>{{ $franchise->ordered_quantity ?? 0 }}</td>
                                                                <td>{{ $franchise->sold_quantity ?? 0 }}</td>
                                                                <td>{{ $franchise->wastage_quantity ?? 0 }}</td>
                                                                <td>{{ ($franchise->ordered_quantity ?? 0) - (($franchise->sold_quantity ?? 0) + ($franchise->wastage_quantity ?? 0)) }}</td>
                                                            </tr>
                                                        @endforeach
                                                    @else
                                                        <tr>
                                                            <td><a href="{{ route('admin.franchises.show', $report->id) }}">{{ $report->name }} {{ $report->lastname }}</a></td>
