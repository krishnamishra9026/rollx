            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        ...
                        <a href="{{ route('admin.account-users.create') }}" class="btn btn-sm btn-dark float-end"><i
                                class="mdi mdi-plus"></i> Add Account User</a>
                    ...
                    <h4 class="page-title">Account Users</h4>
                </div>
            </div>
        </div>
        @include('admin.includes.flash-message')
        @include('admin.user-management.account-users.filter')
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12 table-responsive">
                                                ...
                                                <td>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input checkbox-row"
                                                            name="rows" id="customCheck{{ $account_user->id }}"
                                                            value="{{ $account_user->id }}">
                                                        <label class="form-check-label"
                                                            for="customCheck{{ $account_user->id }}">&nbsp;</label>
                                                    </div>
                                                </td>
                                                <td class="table-user">

                                                    <img @isset($account_user->avatar) src="{{ asset('storage/uploads/admin/' . $admin->avatar) }}" @else src="{{ asset('assets/images/users/avatar.png') }}" @endisset
                                                        alt="table-user" class="me-1 rounded-circle" width="30px">
                                                    <a href="javascript:void(0);"
                                                        class="text-body fw-semibold">{{ $account_user->firstname }}
                                                        {{ $account_user->lastname }}</a>
                                                </td>
                                                <td>{{ $account_user->email }}</td>
                                                <td>{{ $account_user->phone }}</td>
                                                <td><input type="checkbox" id="switch{{ $account_user->id }}"
                                                        @if ($account_user->status == true) checked @endif
                                                        data-switch="success" value="{{ $account_user->id }}"
                                                        class="status" />
                                                    <label for="switch{{ $account_user->id }}" data-on-label="Yes"
                                                ...
                                                <td class="text-end">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <a href="javascript:void(0);" class="dropdown-item change-password"
                                                            ...
                                                            data-id="{{ $account_user->id }}"
                                                            data-name="{{ $account_user->firstname }} {{ $account_user->lastname }}"><i
                                                                class="fa fa-lock me-1"></i> Change Password</a>
                                                        <a href="{{ route('admin.account-users.show', $account_user->id) }}"
                                                            class="dropdown-item"><i class="fa fa-eye me-1"></i>
                                                            View
                                                            User</a>
                                                        <a href="{{ route('admin.account-users.edit', $account_user->id) }}"
                                                            class="dropdown-item"><i class="fa fa-edit me-1"></i>
                                                            Edit
                                                            User</a>
                                                        <a href="javascript:void(0);"
                                                            onclick="confirmDelete({{ $account_user->id }})"
                                ...
                                {{ $account_users->appends(request()->query())->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" id="changePasswordForm"
                        action="{{ route('admin.account-users.reset-password') }}">
                        @csrf
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#delete-all").click(function(e) {
            rows = [], $("input:checkbox[name=rows]:checked").each(function() {
                ...
                t.isConfirmed && ($("#delete-all").text("Deleting..."), e.preventDefault(), $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('admin.account-users.bulk-delete') }}",
                    data: {
                        admins: rows,
                        _token: "{{ csrf_token() }}"
                    },
        ...
        $(".status").change(function() {
            var url = '{{ route('admin.account-users.change-status', ':id') }}';
