...
@section('content')
    <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('admin.technicians.create') }}" class="btn btn-sm btn-dark float-end"><i
        ...
        @include('admin.technicians.filter')
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12 table-responsive">
                                                ...
                                                <td>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input checkbox-row"
                                                            name="rows" id="customCheck{{ $technician->id }}"
                                                            value="{{ $technician->id }}">
                                                        <label class="form-check-label"
                                                            for="customCheck{{ $technician->id }}">&nbsp;</label>
                                                    </div>
                                                </td>
                                                <td class="table-user">

                                                    <img @isset($technician->avatar) src="{{ asset('storage/uploads/technican/' . $technician->avatar) }}" @else src="{{ asset('assets/images/users/avatar.png') }}" @endisset
                                                        alt="table-user" class="me-2 rounded-circle">
                                                    <a href="{{ route('admin.technicians.show', $technician->id) }}"
                                                        class="text-body fw-semibold">{{ $technician->firstname }}
                                                        {{ $technician->lastname }}</a>
                                                </td>                                                
                                                ...
                                                <td class="text-end">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">                                                        
                                                        <a href="javascript:void(0);" class="dropdown-item change-password"
                                                            ...
                                                            data-name="{{ $technician->firstname }} {{ $technician->lastname }}"><i
                                ...
                                {{ $technicians->appends(request()->query())->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" id="changePasswordForm" action="{{ route('admin.technicians.reset-password') }}">
                        @csrf
        ...
        $("#delete-all").click(function(e) {
            rows = [], $("input:checkbox[name=rows]:checked").each(function() {
                ...
                t.isConfirmed && ($("#delete-all").text("Deleting..."), e.preventDefault(), $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('admin.technicians.bulk-delete') }}",
                    data: {
                        technicians: rows,
                        _token: "{{ csrf_token() }}"
                    },
