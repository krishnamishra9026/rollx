                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="{{ route('franchise.dashboard') }}" class="text-dark">Dashboard</a></li>
            ...
            <div class="card">
                <div class="card-body">
                    <form id="accountForm" method="POST"
                        action="{{ route('franchise.my-account.update', Auth::guard('franchise')->id()) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-2 {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            @error('phone')
                                <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                            <input id="dial-code" name="dialcode" type="hidden" value="{{ isset($franchise) ? $franchise->dialcode : '' }}">
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            @endif
                            <img id="preview_img" src="{{ $franchise->avatar }}" class="mt-2" width="100"
                                height="100" />
                        </div>
                    </form>
                </div>
...
@push('scripts')
    <script>
     ...
     <script>
        ...
        @isset($franchise)
            @isset($franchise->iso2)
                countryDropdown.value = '{{ $franchise->iso2 }}';
                iti.setCountry('{{ $franchise->iso2 }}');
            @else
