...
@section('content')
    <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('franchise.chefs.create') }}" class="btn btn-sm btn-dark float-end ms-1"><i
        ...
        @include('franchise.chefs.filter')
        <div class="row">
            <div class="col-12">
                <div class="card">
                                        ...
                                        @foreach ($chefs as $chef)
                                            <tr>
                                                ...
                                                <td class="table-user">
                                                    ...
                                                    <img @isset($chef->avatar) src="{{ asset('storage/uploads/technican/' . $chef->avatar) }}" @else src="{{ asset('assets/images/users/avatar.png') }}" @endisset
                                                        alt="table-user" class="me-2 rounded-circle">
                                                    <a href="{{ route('franchise.chefs.show', $chef->id) }}"
                                                        class="text-body fw-semibold">{{ $chef->firstname }}
                                                        {{ $chef->lastname }}</a>
                                                </td>                                                
                                                <td>{{ $chef->email }}</td>
                                                <td>{{ $chef->phone }}</td>
                                                <td>
                                                    <a href="{{ route('franchise.sales.index', ['chef' => $chef->id]) }}"> {{ $chef->sales->count() }}</a>
                                                </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" id="changePasswordForm" action="{{ route('franchise.chefs.reset-password') }}">
                        @csrf
        ...
        $("#delete-all").click(function(e) {
            rows = [], $("input:checkbox[name=rows]:checked").each(function() {
                ...
                t.isConfirmed && ($("#delete-all").text("Deleting..."), e.preventDefault(), $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('franchise.chefs.bulk-delete') }}",
                    data: {
                        chefs: rows,
                        _token: "{{ csrf_token() }}"
                    },
